...
//----------------------------------------------------------------------------------------error
    XFile? pik = await ImagePicker().pickImage(source: source);
    if(pik != null) {
      setState(() {
        imageFile = pik as File?;
      });
    }
  }
//-----------------------------------------------------------------------------------------end
  ...
  void checkValues() {
    ...
    if(fullname == "") {
      print("Please fill youe name");
      UIHelper.showAlertDialog(context, "Incomplete Data", "fill your name");
    }
    else {
      log("Uploading data..");
      uploadData();
